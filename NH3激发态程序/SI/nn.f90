...
module nn_class
  ...
  !-----------------------------------------------------------------------------
  subroutine init_wb(this,x1,x2)    !初始化w,b
        ![x1,x2]
        implicit none
        class(nn) :: this
  ...
  subroutine calc_dxdydn(this)
        ...
        do l=1,R
              do i=M,2,-1
                    ...
                    call dgemm('T','N',o,SM,p,1.d0,this%w(1:p,1:o,i),p,&
                                this%dydn(1:p,1:SM,i),p,0.d0,wk(1:o,1:SM),o)  !dgemm计算矩阵乘法
                    do k=1,SM
  ...
  subroutine wb_inout(this,id,dx)
        ...
        p=0
        do k=1,M  !遍历神经网络总层数，k为当前层
              q=this%S(k)     !当前层的神经元个数
              !w
              do l=1,this%S(k-1)    !遍历前几层的神经元
                    if(id.eq.0) then !in
                          this%w(1:q,l,k)=dx(p+1:p+q) !dx用一维数组的形式储存了全部的权重
                    else !out
  ...
  !---------------------------------------------------------------------------
  subroutine savepara(this,filename,io)   !保存或加载参数
    implicit none
    class(nn) :: this
    ...
    allocate(para(this%L))    !this%L 是参数总数（权重+偏置）
    if(io.eq.0) then
      !read parameters
      call FLUnit(fid)  !找到未打开的单元
      open(unit=fid,file=trim(adjustl(filename)),access='sequential',&
      form='formatted',STATUS='OLD',ACTION='READ',POSITION='REWIND',IOSTAT=ios)
      if(ios.ne.0) stop 'Error opening NN parameter file!'
      do i=1,this%L
        read(fid,*,iostat=ios) para(i)    !逐行读取参数
        if(ios.ne.0) stop 'Error reading NN parameter file!'      !这里为啥还要再加一个判断，不是已经判断过了吗
      end do
      call this%wb(0,para)    !调用 wb 方法，将参数加载到神经网络
    else    !不为0则保存文件
      !save parameters
      call FLUnit(fid)
      open(unit=fid,file=trim(adjustl(filename)),iostat=ios)
      if(ios.ne.0) stop 'Error writing NN parameter file!'
      call this%wb(1,para)    !调用 wb 方法，将神经网络参数写入到 para 数组中
      do i=1,this%L
        write(fid,*) para(i)
      end do
    end if
...
!=================================================================================
subroutine check_act(activate)      !判断输入的activate（激活函数名称）是否符合要求
      implicit none
      character(len=*), intent(in) :: activate

      if(trim(adjustl(activate)).ne.'tansig' .and. &   !trim,adjustl,格式化内容，去除前后空格
         trim(adjustl(activate)).ne.'algebraic' .and. &     !&是续行符
...
!=================================================================================
subroutine trans(M,n,a,activate)    !使用不同激活函数的前向计算
      implicit none
      real*8, parameter :: fac=2.d0/acos(-1.d0)
      real*8, parameter :: fac1=0.5d0*acos(-1.d0)
      integer, intent(in) :: M
      real*8, intent(in) :: n(M)    !输入
      real*8, intent(out) :: a(M)   !输出
      character(len=*), intent(in) :: activate  !激活函数名称

      if(trim(adjustl(activate)).eq.'tansig') then    !双曲正切函数
            a=tanh(n)
      else if(trim(adjustl(activate)).eq.'algebraic') then  !代数函数
            a=n/sqrt(abs(1.d0+n*n))
      else if(trim(adjustl(activate)).eq.'purelin') then    !线性函数
            a=n
      else if(trim(adjustl(activate)).eq.'logsig') then     !对数sigmoid函数
            a=exp(n)
            a=a/(1.d0+a)
      else if(trim(adjustl(activate)).eq.'arctan') then     !反正切函数
            a=fac*atan(fac1*n)
      else
            stop 'Wrong Activation Fuction!'
      end if
...
!=================================================================================
subroutine dtrans(M,n,dn,activate)  !使用不同激活函数的导数计算，反向传播的梯度计算
      implicit none
      real*8, parameter :: fac1=0.5d0*acos(-1.d0)
      integer, intent(in) :: M
      ...
      if(trim(adjustl(activate)).eq.'tansig') then    
            dn=1.d0/cosh(n)**2      !双曲正切函数的导数的表达式
...
!==================================================================================
subroutine d2trans(M,n,d2n,activate)      !不同激活函数的二阶导数计算
      implicit none
      real*8, parameter :: fac1=0.5d0*acos(-1.d0)
      integer, intent(in) :: M
      real*8, intent(in) :: n(M)
...
!=================================================================================
subroutine FLUnit(fid)  !找到未打开的单元
      ...
      fid=0
      do i=15,99999
            inquire(UNIT=i,EXIST=unitex,OPENED=unitop)      !inquire的三个参数，单元等于i，exist为存在，opened为打开
            if(unitex .and. .not. unitop) then  !f not优先级比and高，所以是未打开且存在为真
                  fid=i
                  exit
            end if
      end do
...
!==================================================================================
subroutine countline(filename,n)    !统计文件 filename 中的行数，并将结果存储在输出参数 n 中。
      implicit none
      character(len=*), intent(in) :: filename  !intend(in)表示只读
      integer, intent(out) :: n     !intent(out)表示输出参数，需在过程中初始化
      integer :: fid,ios

      call FLUnit(fid)
      open(fid,file=trim(adjustl(filename)),iostat=ios)     !adjustl是将文件左对齐（移除前导空格），trim是移除尾部空格；该步骤即确保格式统一
      if(ios.ne.0) then
        print*, "Error opening ",trim(adjustl(filename))    !文件打开失败，输出文件内容，设置n=0
        n=0
        return
      end if
      ...
      n=0
      do
            read (fid,*,end=10)     !读取文件内容，end=10表示到达文件末尾时跳转到10行;语法read(unit, *)，不特殊指定格式，则每个read语句读取整行，所以需要循环来读完整个文件
            n=n+1
      end do
10    close(fid)
