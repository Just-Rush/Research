...
program main
  ...
  real*8, dimension(:,:), allocatable :: gref
  real*8, dimension(:,:,:), allocatable :: geom,ddip
  real*8 :: r,phi,theta
  real*8 :: eshift
  ...
  allocate(ddip(3,nstates,nstates))
  ...
  do i=1,10
    do j=0,30,5
      ...
      call Evaluatediadip(geom(:,:,i),ddip)
      write(200,"(2f8.3,9f14.8)") r,dble(j),ddip(:,1,1),ddip(:,1,2),ddip(:,2,2)
    end do
  end do
